...
//    Viðbætur: eitt sjónarhorn í viðbót (9) og hægt að snúa
//    líkani í sjónarhorni 1.
//
...
window.onload = function init()
{
    ...
    canvas.addEventListener("mousemove", function(e){
        if(movement) {
    	    spinZ = ( spinZ + (e.offsetX - origX) ) % 360;
            spinY = ( spinY + (e.offsetY - origY) ) % 360;
            origX = e.offsetX;
            origY = e.offsetY;
        }
    } );
...
function render()
{
    ...
    switch( view ) {
	    ...
	    mv = lookAt( vec3(250.0+dist, 0.0, 100.0+height), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 1.0) );
        // Can rotate
